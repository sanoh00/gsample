...
void	DX12Renderer::resorceRender()
{
	...
	_command_list->SetGraphicsRootDescriptorTable(0, heaps[0]->GetGPUDescriptorHandleForHeapStart());	//テクスチャの設定変更
	_command_list->SetGraphicsRootDescriptorTable(1, heaps[1]->GetGPUDescriptorHandleForHeapStart());	//サンプラーの設定変更
	_command_list->SetGraphicsRootConstantBufferView(2, _uniform_buffer->GetGPUVirtualAddress());		//固定
...
ComPtr<ID3D12Resource >	DX12Renderer::createLoadImage(const std::string& in_filename)
{
	...
	D3D12_RESOURCE_DESC desc_resource_tex{};
	desc_resource_tex.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
	desc_resource_tex.Width = (UINT64)image_width;
	desc_resource_tex.Height = (UINT)image_height;
	desc_resource_tex.DepthOrArraySize = 1;
	desc_resource_tex.MipLevels = 1;
	desc_resource_tex.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	desc_resource_tex.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
	desc_resource_tex.SampleDesc.Count = 1;
...
//-------------------------------------------------------------------------
ComPtr<ID3D12Resource>	DX12Renderer::createUniform()
{
	HRESULT hr;
	ComPtr<ID3D12Resource>	ret_resorce;
	{
		D3D12_HEAP_PROPERTIES heap_props_vb{};
		heap_props_vb.Type					= D3D12_HEAP_TYPE_UPLOAD;
		heap_props_vb.CPUPageProperty		= D3D12_CPU_PAGE_PROPERTY_UNKNOWN;
		heap_props_vb.MemoryPoolPreference	= D3D12_MEMORY_POOL_UNKNOWN;
		heap_props_vb.CreationNodeMask		= 0;
		heap_props_vb.VisibleNodeMask		= 0;

		D3D12_RESOURCE_DESC   desc_resource_vb{};
		desc_resource_vb.Dimension			= D3D12_RESOURCE_DIMENSION_BUFFER;
		desc_resource_vb.Width				= sizeof(UniformData);
		desc_resource_vb.Height				= 1;
		desc_resource_vb.DepthOrArraySize	= 1;
		desc_resource_vb.MipLevels			= 1;
		desc_resource_vb.Format				= DXGI_FORMAT_UNKNOWN;
		desc_resource_vb.Layout				= D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
		desc_resource_vb.SampleDesc.Count	= 1;

		hr = _device->CreateCommittedResource(&heap_props_vb,
			D3D12_HEAP_FLAG_NONE,
			&desc_resource_vb,
			D3D12_RESOURCE_STATE_GENERIC_READ,
			nullptr,
	...
	{
		void* mapped = nullptr;
		hr = ret_resorce->Map(0, nullptr, &mapped);
		assert(SUCCEEDED(hr));

		UniformData	uni_data = { {1.0,1.0, 0.0, 1.0} };
		memcpy(mapped, &uni_data, sizeof(UniformData));
		ret_resorce->Unmap(0, nullptr);
	}
	return	ret_resorce;
}